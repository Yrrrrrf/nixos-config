...
def current []: nothing -> string {
    capture { asusctl profile get } | lines | where {|l| $l | str starts-with "Active profile:"} | get 0? | default "" | parse "Active profile: {p}" | get p.0? | default "Unknown"
}
# Single source of truth: profile name → presentation record.
# tooltip carries the *description*, not a restatement of `text`.
def meta [profile: string]: nothing -> record { match $profile {

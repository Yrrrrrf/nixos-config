...
def fio-run [
    ...
    let raw = (^fio $"--name=($name)" $"--rw=($rw)" $"--bs=($bs)" $"--size=($TEST_SIZE)" $"--iodepth=($iodepth)" $"--numjobs=($numjobs)" $"--runtime=($RUNTIME_SEC)" --time_based --direct=1 --group_reporting --ioengine=io_uring $"--filename=($test_file)" --output-format=json | from json)
    let job = $raw.jobs.0

...
export def check_exec [name: string, cmd: string, args: list<any> = []] {
...
export def field_issues [rec: record, required: list<any>]: nothing -> list<any> {
    $required | each {|f|
        let v = ($rec | get -o $f)
...
export def report [title: string, results: list<any>]: nothing -> bool {

...
def drive-health []: nothing -> any {
    ...
    } else { 0 }
    {
        ...
        critical_warning: (
        # ── Status ────────────────────────────────────────────────
        $log | get -o critical_warning | default 0)
    }
}
def partition-layout []: nothing -> any {
    let dev = (first-nvme)
    if ($dev | is-empty) { return null }
    let base = ($dev | path basename)
    ...
    let parts = (
        $data | get -o blockdevices | first | get -o children | default []
    )

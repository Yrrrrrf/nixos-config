...
def free-ram-mib []: nothing -> int {
    open /proc/meminfo
    | parse --regex '(?P<key>\w+):\s+(?P<value>\d+)\s+kB'
    | where key == "MemAvailable"
    | get 0.value
    | into int
    | $in / 1024
    | math floor
...
def main [--quick] {
    ...
    let size = if $quick { [$auto_size $QUICK_CAP_MIB] | math min } else { $auto_size }
    let loops = if $quick { 1 } else { 2 }

    let mt_res = (run-memtester $size $loops)
    let mt_label = (["memtester \(" ($size | into string) "M x " ($loops | into string) "\)"] | str join)
